const books = [
  {
    id: 1,
    name: 'As Crônicas de Gelo e Fogo',
    genre: 'Fantasia',
    author: {
      name: 'George R. R. Martin',
      birthYear: 1948,
    },
    releaseYear: 1991,
  },
  {
    id: 2,
    name: 'O Senhor dos Anéis',
    genre: 'Fantasia',
    author: {
      name: 'J. R. R. Tolkien',
      birthYear: 1892,
    },
    releaseYear: 1954,
  },
  {
    id: 3,
    name: 'Fundação',
    genre: 'Ficção Científica',
    author: {
      name: 'Isaac Asimov',
      birthYear: 1920,
    },
    releaseYear: 1951,
  },
  {
    id: 4,
    name: 'Duna',
    genre: 'Ficção Científica',
    author: {
      name: 'Frank Herbert',
      birthYear: 1920,
    },
    releaseYear: 1965,
  },
  {
    id: 5,
    name: 'A Coisa',
    genre: 'Terror',
    author: {
      name: 'Stephen King',
      birthYear: 1947,
    },
    releaseYear: 1986,
  },
  {
    id: 6,
    name: 'O Chamado de Cthulhu',
    genre: 'Terror',
    author: {
      name: 'H. P. Lovecraft',
      birthYear: 1890,
    },
    releaseYear: 1928,
  },
];

// Adicione o código do exercício aqui:

function authorBornIn1947(arr) {
  if (!Array.isArray(arr)) return 'Insira um array'
  return arr.find((book) => book.author.birthYear === 1947).author.name;
}

// console.log(authorBornIn1947(books)) // Stephen King;

function smallerName(arr) {
  if (!Array.isArray(arr)) return 'Insira um array'
  let nameBook = arr[0].name || 'Nenhum livro encontrado';
  // escreva aqui o seu código

  arr.forEach((element) => {
    if (element.name.length < nameBook.length) nameBook = element.name;
  });

  // Variável nameBook que receberá o valor do menor nome;
  return nameBook;
}

// console.log(smallerName(books)); // Duna

function getNamedBook(arr) {
  if (!Array.isArray(arr)) return 'Insira um array';
  return arr.find((element) => element.name.length === 26);
}

// console.log(getNamedBook(books));
/*
  id: 1,
  name: 'As Crônicas de Gelo e Fogo',
  genre: 'Fantasia',
  author: {
        name: 'George R. R. Martin',
    birthYear: 1948,
  },
    releaseYear: 1991,
*/

function booksOrderedByReleaseYearDesc(arr) {
  if (!Array.isArray(arr)) return 'Insira um array';
  return arr.sort((elementA, elementB) => elementA.releaseYear - elementB.releaseYear);
}

// console.log(booksOrderedByReleaseYearDesc(books));
/*
[
  {
    id: 6,
    name: 'O Chamado de Cthulhu',
    genre: 'Terror',
    author: { name: 'H. P. Lovecraft', birthYear: 1890 },
    releaseYear: 1928
  },
  {
    id: 3,
    name: 'Fundação',
    genre: 'Ficção Científica',
    author: { name: 'Isaac Asimov', birthYear: 1920 },
    releaseYear: 1951
  },
  {
    id: 2,
    name: 'O Senhor dos Anéis',
    genre: 'Fantasia',
    author: { name: 'J. R. R. Tolkien', birthYear: 1892 },
    releaseYear: 1954
  },
  {
    id: 4,
    name: 'Duna',
    genre: 'Ficção Científica',
    author: { name: 'Frank Herbert', birthYear: 1920 },
    releaseYear: 1965
  },
  {
    id: 5,
    name: 'A Coisa',
    genre: 'Terror',
    author: { name: 'Stephen King', birthYear: 1947 },
    releaseYear: 1986
  },
  {
    id: 1,
    name: 'As Crônicas de Gelo e Fogo',
    genre: 'Fantasia',
    author: { name: 'George R. R. Martin', birthYear: 1948 },
    releaseYear: 1991
  }
]
*/

function everyoneWasBornOnSecXX(arr) {
  if (!Array.isArray(arr)) return 'Insira um array';
  return arr.every((element) => (element.author.birthYear >= 1901 && element.author.birthYear <= 2000));
}

// console.log(everyoneWasBornOnSecXX(books)); // false

function someBookWasReleaseOnThe80s(arr) {
  if (!Array.isArray(arr)) return 'Insira um array';
  return (arr.find((element) => (element.releaseYear >= 1980 && element.releaseYear <= 1989))) ? true : false;
}

// console.log(someBookWasReleaseOnThe80s(books)) // true

function authorUnique(arr) {
  if (!Array.isArray(arr)) return 'Insira um array';
  const newArray = [];
  arr.forEach((element) => {
    newArray.push(element.author.birthYear);
  });
  return !newArray.filter((item, index) => newArray.indexOf(item) !== index);
}
// ref: https://flexiple.com/javascript/find-duplicates-javascript-array/
console.log(authorUnique(books)); // false
